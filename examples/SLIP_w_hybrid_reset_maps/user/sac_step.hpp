...
namespace sac {
  ...
  inline void sac_step::SimInitXRho( const double &t0, const state_type &x0, 
    ...
    if (events.size() < 1) // if no events
      ...
      // filter out any events detected after tf
      for (size_t i = 0; i < events.size(); i++) {
	if (events.back() > tf)
	  events.pop_back();
      }
      
      events.push_back( tf ); // now there is at least 1 element in events

      while ( events.size() > 1 ) { // while there are at least 2

	/* integrate from tf (or last t_event^-) to t_event^+ */
    	steps_ = simRho( rho_dot_, rho_, *(events.rbegin()+1)+1E-6, 
	...
	x_intp( *(events.rbegin()+1)+1E-6, s_x_plus );
	// get f(t_event^-) and f(t_event^+)
	xdot_( s_x_minus, s_f_minus, *(events.rbegin()+1) );
	xdot_( s_x_plus, s_f_plus, *(events.rbegin()+1)+1E-6 );
	//
	...
	v_rho_plus=(Pi(s_x_minus,v_f_minus,v_f_plus,s_x_plus[8])).transpose()
