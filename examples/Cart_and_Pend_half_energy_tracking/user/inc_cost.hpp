  ...
  public:
    ...
    */
    void operator() (const state_type &/*J*/, state_type &dJdt, const double t)
    {
      ...
      //
      // Energy = 1/2 m*r^2*th_dot^2 + g*h*(cos(th)+1)
      //
      // dJdt[0] = pow( ((4.0*m_x[1]*m_x[1])/2.0 + 9.81*2.0*(cos(m_x[0])+1) 
      // 		       - 9.81*4.0) , 2.0);
      dJdt[0] = 0;
      //
    ...
    */
    inline void grad( const double t, const vec_type &mx,
      ...
      // commult=(19.62*(-1.0 + cos(m_x[0])) + 2.0*m_x[1]*m_x[1]);
      // Glofx(0,0) = -39.24*sin(m_x[0])*commult;
      // Glofx(1,0) = 8.0*m_x[1]*commult;
      Glofx << 0,0;
      //
