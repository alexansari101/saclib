  ...
  public:
    ...
    */
    inline void grad( const double t, const vec_type &mx,
      ...
      // p_.x_des( t, m_x, m_mxdes ); // Get desired trajectory point
      // //
      // Glofx = (mgproj_x_ - Gxdes(m_x[0], m_x[2]))*p_.Q()*(mx-m_mxdes);

      // // p_.dinc_x_cost( t, mx, dldx );


      // finite difference
      static state_type sx(p_.xlen()); 
      static vec_type vx = vec_type::Zero(p_.xlen(),1);
      static double Jx, Jx1, Jx2, Jx3, Jx4, dx = .001;//.108;
      Mat2State( mx, m_x);      
      p_.x_des( t, m_x, m_mxdes ); // Get desired trajectory point
      Jx = ( ( (mx-m_mxdes).transpose() * p_.Q() 
      		    * (mx-m_mxdes) ) / 2.0 )(0);
      // backward differences
      sx=m_x; sx[0] = sx[0]-dx;
      p_.proj( sx ); State2Mat( sx, vx);
      p_.x_des( t, sx, m_mxdes );
      Glofx(0,0) = (Jx - ( ( (vx-m_mxdes).transpose() * p_.Q() 
			     * (vx-m_mxdes) ) / 2.0 )(0) )/dx;
      //
      sx=m_x; sx[1] = sx[1]-dx;
      p_.proj( sx ); State2Mat( sx, vx);
      p_.x_des( t, sx, m_mxdes );
      Glofx(1,0) = (Jx - ( ( (vx-m_mxdes).transpose() * p_.Q() 
			     * (vx-m_mxdes) ) / 2.0 )(0) )/dx;
      //
      sx=m_x; sx[2] = sx[2]-dx;
      p_.proj( sx ); State2Mat( sx, vx);
      p_.x_des( t, sx, m_mxdes );
      Glofx(2,0) = (Jx - ( ( (vx-m_mxdes).transpose() * p_.Q() 
			     * (vx-m_mxdes) ) / 2.0 )(0) )/dx;
      //
      sx=m_x; sx[3] = sx[3]-dx;
      p_.proj( sx ); State2Mat( sx, vx);
      p_.x_des( t, sx, m_mxdes );
      Glofx(3,0) = (Jx - ( ( (vx-m_mxdes).transpose() * p_.Q() 
			     * (vx-m_mxdes) ) / 2.0 )(0) )/dx;
